...
import { saveUrlPosition, selectPeriod, markVideoStarted } from "../helpers/navigation";
        ...
        return (
            <div className={css.hbox(8).alignItems("center")}>
                <button className={playBtnCss} title="Play/Pause (space)"
                    onMouseDown={(e: any) => { if (e.button !== 0) return; e.preventDefault(); markVideoStarted(); player?.togglePlay(); }}>
                    {playing ? "❚❚" : "►"}
                </button>
                ...
                <button className={liveBtnCss} title="Seek to the latest recorded time (now)"
                    onMouseDown={(e: any) => { if (e.button !== 0) return; e.preventDefault(); markVideoStarted(); void selectPeriod(Date.now(), true, Date.now()); }}>⇥ Now</button>
