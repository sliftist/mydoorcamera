...
function atSuffix(): string { return state.activityThreshold !== 0.001 ? `&at=${state.activityThreshold}` : ""; }
...
export function getUrlThreshold(): number { try { const n = Number(new URLSearchParams(location.search).get("at")); return n > 0 && n <= 1 ? n : 0.001; } catch { return 0.001; } }
